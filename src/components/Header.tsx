  ...
  if (isLightMode) {
    return (
      <div className="lf-header">
        <img
          className="lf-header__image-delete"
          src={MeltpotLogoNegative}
          alt="Logo"
        ></img>
        <div className="lf-header__options">
          <button
            className={
              isPortuguese ? "lf-bold lf-header__options--selected" : "lf-bold"
            }
            onClick={() => {
              changeLanguage();
    ...
    return (
      ...
      >
        <div className="lf-header__container">
          <a className="lf-header__image" href="#">
            <img
              src={isScrolled || isOpen ? MeltpotLogoNegative : MeltpotLogo}
              alt="Logo"
            ></img>
          </a>
          <a className="lf-header__button" onClick={() => handleClick(!isOpen)}>
            <FontAwesomeIcon
              icon={faChevronDown}
              beat
              style={
                isOpen || isScrolled
                  ? { color: "#000000", height: 40, width: 40 }
                  : { color: "#ffffff", height: 40, width: 40 }
              }
            />
          </a>
        </div>
        {isOpen && (
          <nav className="lf-header__nav">
            <a href="#about" onClick={() => handleClick(!isOpen)}>
              {isPortuguese ? "Sobre" : "About"}
            </a>
            <a href="#events" onClick={() => handleClick(!isOpen)}>
              {isPortuguese ? "Cidades" : "Cities"}
            </a>
            <a href="#t&c" onClick={() => handleClick(!isOpen)}>
              T&C
            </a>
            <div className="lf-header__nav-lang">
              <button
                className={
                  isPortuguese
                    ? "lf-bold lf-header__nav-lang--selected"
                    : "lf-bold"
                }
                onClick={() => {
                  changeLanguage();
    ...
    return (
      <div className={isScrolled ? "lf-header lf-header-white" : "lf-header"}>
        {hasSlogan && (
          <a className="lf-header__image" href="#">
            <img
              src={isScrolled ? MeltpotLogoNegative : MeltpotLogo}
              alt="Logo"
            ></img>
          </a>
        )}
